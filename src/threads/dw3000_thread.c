...
// Tunable parameter to determine when to send the responder message upon receiving a message from
// the initiator.  If you get warnings about transmissions getting canceled, make this number
// bigger.  This was the smallest I could get it for my given transmission parameters, preamble,
// etc. (in UWB microseconds):
...
// functions below.  I saw this recommended by Zephyr somewhere; something about in case you update
// the SPI config while it's doing something else, since it needs to be a global pointer for the
// DW3000 driver to be able to use the platform specific SPI functions.
...
// Device tree specification for the SPI peripheral connected to the DW3000.  Initialized to the
// slow frequency by default.
static const struct spi_dt_spec* dw3000_spi = &dw_spi_slow;

static const struct gpio_dt_spec dw3000_reset_pin = GPIO_DT_SPEC_GET(DW3000_NODE, reset_gpios);

static const struct gpio_dt_spec dw3000_irq_pin = GPIO_DT_SPEC_GET(DW3000_NODE, irq_gpios);
...
int dw_readfromspi(
    uint16_t headerLength, uint8_t* headerBuffer, uint16_t readLength, uint8_t* readBuffer)
{
...
int dw_writetospi(uint16_t headerLength, const uint8_t* headerBuffer, uint16_t bodyLength,
...
int dw_writetospiwithcrc(uint16_t headerLength, const uint8_t* headerBuffer, uint16_t bodyLength,
...
/*
   Wake up DW3000 by pulling the chip select pin active for 500 microseconds.  Then wait for 500
   microseconds for the device to wake up.
*/
...
/*
   DW3000 function definitions for SPI communication, which are passed to the DW3000 driver via the
   `struct dwt_probe_s` `dw_probe` by calling `dwt_probe()`.
*/
...
/*
   DW3000 probe structure to be passed to `dwt_probe()` on initialization, which tracks the SPI
   function definitions in `dw_spi_functions`, and the list of available DW drivers in
   `dw_driver_list`.
*/
...
*/
void dw3000_irq_handler(const struct device* port, struct gpio_callback* cb, uint32_t pins)
{
    dwt_isr();
}
...
// IEEE standard message type for a blink is 0xC5, followed by sequence number, then eight byte
// device ID:
...
/*
    Callback function for the DW3000 driver to call when a RX good frame event is triggered.
*/
void dw3000_rxok_callback(const dwt_cb_data_t* data) { k_event_post(&dw3000_events, DW3000_RX_OK); }

/*
    Callback function for the DW3000 driver to call when any error event is triggered.
*/
...
static int dw3000_reset_configure()
{
    ...
    dw3000_config.sfdTO = dw3000_config.txPreambLength + 1 + sfd_length - pac_length;
    ...
    dwt_configuresleep(DWT_CONFIG | DWT_GOTOIDLE, DWT_PRES_SLEEP | DWT_WAKE_CSN | DWT_SLP_EN);
...
/*
   ...
   #### Returns:
    - 0 if wake succeeds.
*/
static int dw3000_wake_from_sleep(void)
{
    dw_wakeup_device_with_io();
...
static int responder_range_reply(struct dw3000_msg_data* responder_message)
{
    ...
    // transmit time only uses the top 31 bits of the 40 bit timestamps, which is why you see it
    // converted to a uint32 by shifting down eight bits, then convert it back and lopping off the
    // LSB.  This way the actual time spent between reception and transmission is accurate to when
    // the actual message is sent:
    uint32_t transmit_time =
        (rx_timestamp + (DW3000_TRANSMIT_TIME_DELAY_UUS * UUS_TO_DWT_TIME)) >> 8;

    dwt_setdelayedtrxtime(transmit_time);

    responder_message->reply_time =
        (((uint64_t)(transmit_time & 0xFFFFFFFEUL)) << 8) + DW3000_ANTENNA_DELAY - rx_timestamp;
    ...
    int32_t error = dwt_starttx(DWT_START_TX_DELAYED);
    if (DWT_SUCCESS != error)
    {
    ...
    LOG_DBG("Transmit time: %llu", (uint64_t)transmit_time << 8);
...
/*
   Waits for a random amount of time less than `DW3000_MAX_WAIT_UNTIL_IDLE_MS`, then replies to an
   initiator pair message.  It then waits for the initiator to ACK that reply.  If it does, it
...
/*
   ...
   For range messages, if it's paired with the initiator, it schedules a time to reply, and then
   sends a reply message.  Included in the reply message is the time it took from receiving the
   message to the time it was sent (based on delayed transmit).
    ...
    struct dw3000_msg_data* responder_message, uint64_t* paired_initiator_id, struct k_timer* timer)
{
    ...
    else if (((uint8_t)DW3000_INIT_PAIR_TYPE == rx_message.msg_type) && (0 == *paired_initiator_id))
    {
...
static int initiator_measure_range(struct dw3000_msg_data* initiator_message)
{
    ...
    double range = (rx_timestamp - tx_timestamp - rx_message.reply_time * (1.0 - offset)) / 2.0 *
                   DWT_TIME_UNITS * SPEED_OF_LIGHT;
    ...
    uint64_t* responder_list, uint8_t* nresponders, uint8_t* ntimeouts)
{
    ...
    do
    {
        ...
        sleep_time_ms =
            sleep_time_ms >= DW3000_MAX_WAIT_PAIR_MS ? 0 : DW3000_MAX_WAIT_PAIR_MS - sleep_time_ms;
        ...
        if ((event & DW3000_RX_OK) != 0)
        {
            ...
            uint8_t ridx = 0;
            for (; ridx < DW3000_MAX_RESPONDERS; ridx++)
            {
                if (rx_message.responder_id == responder_list[ridx])
                {
            ...
            for (ridx = 0; ridx < DW3000_MAX_RESPONDERS; ridx++)
            {
                if (0 == responder_list[ridx])
                    break;
            }

            if (DW3000_MAX_RESPONDERS <= ridx)
                break;
    ...
    *ntimeouts = *ntimeouts > DW3000_MAX_PAIRING_TIMEOUTS ? DW3000_MAX_PAIRING_TIMEOUTS :
                                                            *ntimeouts;
}

/*
   Right now, only two types of messages are initiated: range finding, and pairing.
   ...
   and then it adds it to its `responder_list` (unless it's already there).

   #### Parameters:
    - `initiator_message` - a poiner to the `dw3000_msg_data` to be sent to and from responders.
*/
void manage_initiator_messages(struct dw3000_msg_data* initiator_message)
{
    ...
    static uint8_t nmissed[DW3000_MAX_RESPONDERS] = {0};
    static uint8_t nresponders = 0;
    static uint8_t ntimeouts = 0;

    // for every responder recorded, attempt to get a range once:
    uint8_t nranges = 0;
    for (uint8_t ridx = 0; ridx < DW3000_MAX_RESPONDERS; ridx++)
    {
        if (nresponders <= nranges)
            break;

        if (0 == responder_list[ridx])
            continue;

        initiator_message->responder_id = responder_list[ridx];
        LOG_DBG("DW3000 TX event Device ID = 0x%llx", initiator_message->initiator_id);

        int error = initiator_measure_range(initiator_message);
        if (0 != error)
        {
            nmissed[ridx]++;
            if (DW3000_MAX_NUM_COMM_ERRORS <= nmissed[ridx])
            {
                nmissed[ridx] = 0;
                responder_list[ridx] = 0;
                nresponders--;
                ntimeouts = 0;
            }
    ...
    // have enough responders, but have tried to find more but can't; or, have max:
    if (((ntimeouts >= DW3000_MAX_PAIRING_TIMEOUTS) && (nresponders >= DW3000_MIN_PAIRS)) ||
    ...
    struct k_timer* timeout_timer, struct k_timer* sleep_timer)
{
    while (true)
    {
        uint32_t event = k_event_wait(&dw3000_events,
            DW3000_RX_OK | DW3000_RX_ERR | DW3000_WAKE_UP | DW3000_LOST_CONTACT, true,
            K_MSEC(DW3000_MAX_TIMEOUT_RESP_RX_MS));

        if ((event & DW3000_WAKE_UP) != 0)
        {
            ...
            // ignore any other events and get back to listening.
            continue;
        }
        ...
        {
            int error =
                manage_responder_messages(responder_message, paired_initiator_id, sleep_timer);
            ...
            {
                k_timer_start(timeout_timer, K_MSEC(DW3000_MAX_TIME_WITHOUT_MSG_MS), K_FOREVER);
            }

            // all events will be cleared by this point, just get back to waiting:
            continue;
        }
...
void start_initiator(struct dw3000_msg_data* tx_message)
{
    while (true)
    {
        k_event_wait(&dw3000_events, DW3000_TX_START, true, K_FOREVER);
...
void dw3000_thread(void* initiator, void* unused0, void* unused1)
{
    ...
    if (is_initiator)
    {
        ...
        k_timer_init(&dw3000_timer, dw3000_tx_timer_expires, NULL);
        k_timer_start(&dw3000_timer, K_SECONDS(1), K_SECONDS(1));

        start_initiator(&tx_message);
    }
    else
    {
        ...
        start_responder(
            &tx_message, &paired_initiator_id, &dw3000_responder_timeout, &dw3000_timer);
    }
}
