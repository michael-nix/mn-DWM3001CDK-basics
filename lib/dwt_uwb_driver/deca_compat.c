 ...
 * @file:     deca_compat.c
 *
 * @brief     This file is a wrapper which sits on top of the low-level driver
 * file, it will call either DW3000 or DW3720 APIs depending which device was
 * discovered by the dwt_probe() function.
 *
 * @note Requirements to the application:
 * Call the dwt_probe() at the start of the application to select the correct
 * driver.
 *
...
/*! Use statically allocated struct: to make driver compatible with legacy
 * implementations: 1 chip <-> 1 driver */
 ...
 *
 * @return ret, @ref DWT_ERROR if no driver found or invalid probe_interface.
 * @ref DWT_SUCCESS if driver is found.
 */
int32_t dwt_probe(struct dwt_probe_s* probe_interf)
{
    ...
    if (probe_interf != NULL)
    {
        ...
        devId = (((uint32_t)buf[3] << 24UL) | ((uint32_t)buf[2] << 16UL) |
                 ((uint32_t)buf[1] << 8UL) | (uint32_t)buf[0]);

#ifdef WIN32
        // Find which DW device the host is connected to and assign correct
        // low-level driver structure
        for (uint8_t i = 0U; i < 2U; i++)
        {
            if ((devId & tmp_ptr[i]->devmatch) ==
                (tmp_ptr[i]->devid & tmp_ptr[i]->devmatch))
            {
        ...
        {
            if ((devId & driver_list[i]->devmatch) ==
                (driver_list[i]->devid & driver_list[i]->devmatch))
            {
...
/*!
 * ------------------------------------------------------------------------------------------------------------------
 * @brief This function will update dw pointer to the @ref dwchip_s structure
 * used by the driver APIs. There is a static struct dwchip_s static_dw = { 0 };
 * in the deca_compat.c file and a pointer to it, *dw.
 *
 * @param[in] new_dw: pointer to dw @ref dwchip_s structure instatiated by MCPS
 * layer
 *
 * @return pointer to the old device structure dwchip_s, which can be restored
 * when deleting MCPS instance
 *
 */
...
int32_t dwt_apiversion(void) { return (int32_t)dw->dwt_driver->vernum; }

const char* dwt_version_string(void) { return DRIVER_VERSION_STR; }
...
{
    uint32_t tmp = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READ_REG, DW3XXX_DEVICE_ID, (void*)&tmp);
    return tmp;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETPLLCALTEMP, temperature, NULL);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETFINEGRAINTXSEQ, enable, NULL);
}
...
int32_t dwt_restoreconfig(dwt_restore_type_e restore_mask)
{
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_RESTORECONFIG, (int32_t)restore_mask, NULL);
}
...
{
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_RESTORETXRX, (uint8_t)restore_mask, NULL);
}

void dwt_configurestsmode(uint8_t stsMode)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGURESTSMODE, 0, (void*)&stsMode);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETRXANTENNADELAY, 0, (void*)&rxAntennaDelay);
}
...
uint16_t dwt_getrxantennadelay(void)
{
    uint16_t tmp = 0U;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_GETRXANTENNADELAY, 0, (void*)&tmp);
    return tmp;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETTXANTENNADELAY, 0, (void*)&txAntennaDelay);
}
...
uint16_t dwt_gettxantennadelay(void)
{
    uint16_t tmp = 0U;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_GETTXANTENNADELAY, 0, (void*)&tmp);
    return tmp;
}

int32_t dwt_writetxdata(
    uint16_t txDataLength, uint8_t* txDataBytes, uint16_t txBufferOffset)
{
    return dw->dwt_driver->dwt_ops->write_tx_data(
        dw, txDataLength, txDataBytes, txBufferOffset);
}

void dwt_writetxfctrl(
    uint16_t txFrameLength, uint16_t txBufferOffset, uint8_t ranging)
{
    dw->dwt_driver->dwt_ops->write_tx_fctrl(
        dw, txFrameLength, txBufferOffset, ranging);
}

int32_t dwt_setplenfine(uint16_t preambleLength)
{
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETPLENFINE, 0, (void*)&preambleLength);
}

int dwt_setpllrxprebufen(dwt_pll_prebuf_cfg_e pll_rx_prebuf_cfg)
{
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETPLLRXPREBUFEN, 0, (void*)&pll_rx_prebuf_cfg);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETREFERENCETRXTIME, 0, (void*)&reftime);
}

void dwt_setdelayedtrxtime(uint32_t starttime)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETDELAYEDTRXTIME, 0, (void*)&starttime);
}
...
uint8_t dwt_get_dgcdecision(void)
{
    uint8_t tmp = 0U;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_GETDGCDECISION, 0, (void*)&tmp);
    return tmp;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READTXTIMESTAMP, 0, (void*)timestamp);
}
...
uint32_t dwt_readtxtimestamphi32(void)
{
    uint32_t tmp = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READTXTIMESTAMPHI32, 0, (void*)&tmp);
    return tmp;
}
...
uint32_t dwt_readtxtimestamplo32(void)
{
    uint32_t tmp = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READTXTIMESTAMPLO32, 0, (void*)&tmp);
    return tmp;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READRXTIMESTAMPUNADJ, 0, (void*)timestamp);
}

void dwt_readrxtimestamp_ipatov(uint8_t* timestamp)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READRXTIMESTAMP_IPATOV, 0, (void*)timestamp);
}

void dwt_readrxtimestamp_sts(uint8_t* timestamp)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READRXTIMESTAMP_STS, 0, (void*)timestamp);
}
...
uint32_t dwt_readrxtimestamphi32(void)
{
    uint32_t tmp = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READRXTIMESTAMPHI32, 0, (void*)&tmp);
    return tmp;
}
...
uint32_t dwt_readrxtimestamplo32(dwt_ip_sts_segment_e segment)
{
    ...
    (void)segment; // not used
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READRXTIMESTAMPLO32, 0, (void*)&tmp);
    return tmp;
}
...
uint32_t dwt_readsystimestamphi32(void)
{
    uint32_t tmp = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READSYSTIMESTAMPHI32, 0, (void*)&tmp);
    return tmp;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READSYSTIME, 0, (void*)timestamp);
}
...
void dwt_setdblrxbuffmode(
    dwt_dbl_buff_state_e dbl_buff_state, dwt_dbl_buff_mode_e dbl_buff_mode)
{
    struct dwt_set_dbl_rx_buff_mode_s tmp = {dbl_buff_state, dbl_buff_mode};
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETDBLRXBUFFMODE, 0, (void*)&tmp);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETRXTIMEOUT, 0, (void*)&rx_time);
}

void dwt_setpreambledetecttimeout(uint16_t timeout)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETPREAMBLEDETECTTIMEOUT, 0, (void*)&timeout);
}
...
uint16_t dwt_calibratesleepcnt(void)
{
    uint16_t tmp = 0U;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CALIBRATESLEEPCNT, 0, (void*)&tmp);
    return tmp;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGURESLEEPCNT, 0, (void*)&sleepcnt);
}
...
void dwt_configuresleep(uint16_t mode, uint8_t wake)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGURESLEEP, 0, (void*)&tmp);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_ENTERSLEEPAFTERTX, enable, NULL);
}

void dwt_entersleepafter(int32_t event_mask)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_ENTERSLEEPAFTER, event_mask, NULL);
}
...
void dwt_setinterrupt(
    uint32_t bitmask_lo, uint32_t bitmask_hi, dwt_INT_options_e INT_options)
{
    dw->dwt_driver->dwt_ops->set_interrupt(
        dw, bitmask_lo, bitmask_hi, INT_options);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETADDRESS16, 0, (void*)&shortAddress);
}
...
void dwt_configureframefilter(uint16_t enabletype, uint16_t filtermode)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGUREFRAMEFILTER, 0, (void*)&tmp);
}

uint8_t dwt_generatecrc8(
    const uint8_t* byteArray, uint32_t flen, uint8_t crcInit)
{
#ifdef DWT_ENABLE_CRC
    uint8_t data;
...
void dwt_enablespicrccheck(
    dwt_spi_crc_mode_e crc_mode, dwt_spierrcb_t spireaderr_cb)
{
    struct dwt_enable_spi_crc_check_s tmp = {crc_mode, spireaderr_cb};
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_ENABLESPICRCCHECK, 0, (void*)&tmp);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETRXAFTERTXDELAY, 0, (void*)&rxDelayTime);
}

void dwt_softreset(int32_t reset_semaphore)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SOFTRESET, 0, (void*)&reset_semaphore);
}
...
{
    return dw->dwt_driver->dwt_ops->read_cir(
        dw, buffer, cir_idx, sample_offs, num_samples, mode);
}

int dwt_readcir_48b(uint8_t* buffer, dwt_acc_idx_e acc_idx,
    uint16_t sample_offs, uint16_t num_samples)
{
    // In the QM33 devices the DWT_CIR_READ_FULL is already 48-bit. This
    // function is added only for compatibility with QM35 devices
    return dw->dwt_driver->dwt_ops->read_cir(dw, (uint32_t*)(void*)buffer,
        acc_idx, sample_offs, num_samples, DWT_CIR_READ_FULL);
}
...
int16_t dwt_readclockoffset(void)
{
    int16_t tmp = 0;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READCLOCKOFFSET, 0, (void*)&tmp);
    return tmp;
}
...
int32_t dwt_readcarrierintegrator(void)
{
    int32_t tmp = 0;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READCARRIERINTEGRATOR, 0, (void*)&tmp);
    return tmp;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGCIADIAG, 0, (void*)&enable_mask);
}
...
int32_t dwt_readstsquality(int16_t* rxStsQualityIndex, int32_t stsSegment)
{
    ...
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READSTSQUALITY, 0, (void*)rxStsQualityIndex);
}

int32_t dwt_readstsstatus(uint16_t* stsStatus, int32_t sts_num)
{
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READSTSSTATUS, sts_num, (void*)stsStatus);
}

void dwt_readdiagnostics(dwt_rxdiag_t* diagnostics)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READDIAGNOSTICS, 0, (void*)diagnostics);
}

void dwt_configeventcounters(int32_t enable)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGEVENTCOUNTERS, enable, NULL);
}

void dwt_readeventcounters(dwt_deviceentcnts_t* counters)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READEVENTCOUNTERS, 0, (void*)counters);
}
...
int32_t dwt_otpwriteandverify(uint32_t value, uint16_t address)
{
    ...
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_OTPWRITEANDVERIFY, 0, (void*)&tmp);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_REPEATEDFRAMES, 0, (void*)&framerepetitionrate);
}
...
void dwt_send_test_preamble(uint16_t delay, uint32_t test_txpower)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_REPEATEDPREAMBLE, 0, (void*)&tmp);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGCONTINUOUSFRAMEMODE, 0, (void*)&framerepetitionrate);
}

void dwt_disablecontinuousframemode(void)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_DISABLECONTINUOUSFRAMEMODE, 0, NULL);
}

void dwt_disablecontinuouswavemode(void)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_DISABLECONTINUOUSWAVEMODE, 0, NULL);
}
...
float dwt_convertrawtemperature(uint8_t raw_temp)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONVERTRAWTEMP, 0, (void*)&tmp);
    return tmp.result;
}
...
float dwt_convertrawvoltage(uint8_t raw_voltage)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONVERTRAWVBAT, 0, (void*)&tmp);
    return tmp.result;
}
...
uint8_t dwt_readwakeuptemp(void)
{
    uint8_t tmp = 0U;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READWAKEUPTEMP, 0, (void*)&tmp);
    return tmp;
}
...
uint8_t dwt_readwakeupvbat(void)
{
    uint8_t tmp = 0U;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READWAKEUPVBAT, 0, (void*)&tmp);
    return tmp;
}
...
uint8_t dwt_calcbandwidthadj(uint16_t target_count)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CALCBANDWIDTHADJ, 0, (void*)&tmp);
    return tmp.result;
}
...
dwt_mic_size_e dwt_mic_size_from_bytes(uint8_t mic_size_in_bytes)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_MICSIZEFROMBYTES, 0, (void*)&tmp);
    return tmp.result;
}
...
{
    return (uint32_t)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_PLL_STATUS, 0, NULL);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGURE_RF_PORT, (int)port_control, NULL);
}
...
void dwt_configure_le_address(uint16_t addr, int32_t leIndex)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGURELEADDRESS, addr, (void*)&tmp);
}

void dwt_configuresfdtype(dwt_sfd_type_e sfdType)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGURESFDTYPE, 0, (void*)&sfdType);
}
...
uint32_t dwt_read_reg(uint32_t address)
{
    uint32_t tmp32 = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READ_REG, (int)address, (void*)&tmp32);
    return tmp32;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_WRITE_REG, address, (void*)(uint32_t*)data);
}

void dwt_writesysstatuslo(uint32_t mask)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_WRITESYSSTATUSLO, 0, (void*)&mask);
}

void dwt_writesysstatushi(uint32_t mask)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_WRITESYSSTATUSHI, 0, (void*)&mask);
}
...
uint32_t dwt_readsysstatuslo(void)
{
    uint32_t tmp = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READSYSSTATUSLO, 0, (void*)&tmp);
    return tmp;
}
...
uint32_t dwt_readsysstatushi(void)
{
    uint32_t tmp = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READSYSSTATUSHI, 0, (void*)&tmp);
    return tmp;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_WRITERDBSTATUS, 0, (void*)&mask);
}
...
uint16_t dwt_getframelength(uint8_t* rng)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_GETFRAMELENGTH, 0, (void*)&tmp);
    *rng = tmp.rng_bit;
...
uint32_t dwt_readpdoaoffset(void)
{
    uint32_t tmp = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READPDOAOFFSET, 0, (void*)&tmp);
    return tmp;
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETPDOAOFFSET, 0, (void*)&offset);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_DSENSLEEP, 0, (void*)&host_sleep_en);
}

int32_t dwt_ds_setinterrupt_SPIxavailable(
    dwt_spi_host_e spi_num, dwt_INT_options_e int_set)
{
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_DSSETINT_SPIAVAIL, (int)spi_num, (void*)&int_set);
}

void dwt_enable_disable_eq(uint8_t en)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_ENABLEDISABLEEQ, 0, (void*)&en);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONFIGTIMER, 0, (void*)tim_cfg);
}
...
void dwt_set_timer_expiration(dwt_timers_e timer_name, uint32_t expiration)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_TIMEREXPIRATION, 0, (void*)&tmp);
}

void dwt_timer_enable(dwt_timers_e timer_name)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_TIMERENABLE, 0, (void*)&timer_name);
}
...
void dwt_wifi_coex_set(dwt_wifi_coex_e enable, int32_t coex_io_swap)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CFGWIFICOEXSET, 0, (void*)&tmp);
}
...
uint32_t dwt_getcirregaddress(void)
{
    uint32_t tmp = 0UL;
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_GET_CIR_REGADD, 0, (void*)&tmp);
    return tmp;
}

register_name_add_t* dwt_get_reg_names(void)
{
    return (register_name_add_t*)dw->dwt_driver->dwt_ops->dbg_fn(
        dw, DWT_DBG_REGS, 0, NULL);
}

void dwt_set_alternative_pulse_shape(uint8_t set_alternate)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SET_ALT_PULSE_SHAPE, 0, (void*)&set_alternate);
}

int dwt_nlos_alldiag(dwt_nlos_alldiag_t* all_diag)
{
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_NLOS_ALLDIAG, 0, (void*)all_diag);
}
...
int32_t dwt_adjust_tx_power(uint16_t boost, uint32_t ref_tx_power,
    uint8_t channel, uint32_t* adj_tx_power, uint16_t* applied_boost)
{
    struct dwt_adj_tx_power_s tmp = {
        0, boost, ref_tx_power, channel, adj_tx_power, applied_boost};
...
int32_t dwt_calculate_linear_tx_power(
    uint32_t channel, power_indexes_t* p_indexes, tx_adj_res_t* p_res)
{
    struct dwt_calculate_linear_tx_power_s tmp = {0, channel, p_indexes, p_res};
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_LINEAR_TXPOWER, 0, (void*)&tmp);
    *p_indexes = *tmp.txp_indexes;
    *p_res = *tmp.txp_res;
    return tmp.result;
}

int dwt_convert_tx_power_to_index(
    uint32_t channel, uint8_t tx_power, uint8_t* tx_power_idx)
{
    struct dwt_convert_tx_power_to_index_s tmp = {
        0, channel, tx_power, tx_power_idx};
    int ret = dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CONVERT_TXPOWER_TO_IDX, 0, (void*)&tmp);
    if (ret != 0)
    {
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SET_PLLBIASTRIM, 0, (void*)&pll_bias_trim);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SET_STS_LEN, 0, (void*)&stsblocks);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SET_DATARATE, 0, (void*)&bitRate);
...
uint8_t dwt_pll_chx_auto_cal(int32_t chan, uint32_t coarse_code, uint16_t sleep,
    uint8_t steps, int8_t temperature)
{
    struct dwt_set_pll_cal_s tmp = {coarse_code, sleep, steps, temperature};
    return ((uint8_t)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_PLL_AUTO_CAL, chan, (void*)&tmp));
}

int32_t dwt_xtal_temperature_compensation(
    dwt_xtal_trim_t* params, uint8_t* xtaltrim)
{
    struct dwt_set_xtal_cal_s tmp = {params, xtaltrim};
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_XTAL_AUTO_TRIM, 0, (void*)&tmp);
}

void dwt_capture_adc_samples(dwt_capture_adc_t* capture_adc)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CAPTURE_ADC, 0, (void*)capture_adc);
}

void dwt_read_adc_samples(dwt_capture_adc_t* capture_adc)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READ_ADC_SAMPLES, 0, (void*)capture_adc);
}

void dwt_configtxrxfcs(uint8_t enable)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SET_FCS_MODE, 0, (void*)&enable);
}

int dwt_calculate_rssi(
    const dwt_cirdiags_t* diag, dwt_acc_idx_e acc_idx, int16_t* signal_strength)
{
    struct dwt_calculate_rssi_s tmp = {diag, acc_idx, signal_strength};
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CALCULATE_RSSI, 0, (void*)&tmp);
}

int dwt_calculate_first_path_power(
    const dwt_cirdiags_t* diag, dwt_acc_idx_e acc_idx, int16_t* signal_strength)
{
    struct dwt_calculate_rssi_s tmp = {diag, acc_idx, signal_strength};
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_CALCULATE_FIRST_PATH_POWER, 0, (void*)&tmp);
}
...
int dwt_readdiagnostics_acc(dwt_cirdiags_t* diag, dwt_acc_idx_e acc_idx)
{
    ...
    return dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_READDIAGNOSTICS_ACC, 0, (void*)&tmp);
}

int dwt_setpdoamode(dwt_pdoa_mode_e pdoaMode)
{
    return (dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SETPDOAMODE, (int32_t)pdoaMode, NULL));
}

void dwt_configureisr(dwt_isr_flags_e flags)
{
    (void)dw->dwt_driver->dwt_ops->ioctl(
        dw, DWT_SET_ISR_FLAGS, (int32_t)flags, NULL);
}
...
void dwt_writetodevice(
    uint32_t regFileID, uint16_t index, uint16_t length, uint8_t* buffer)
{
    dw->dwt_driver->dwt_mcps_ops->write_to_device(
        dw, regFileID, index, length, buffer);
}

void dwt_readfromdevice(
    uint32_t regFileID, uint16_t index, uint16_t length, uint8_t* buffer)
{
    dw->dwt_driver->dwt_mcps_ops->read_from_device(
        dw, regFileID, index, length, buffer);
}
